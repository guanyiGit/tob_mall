...
public class BuildTree {
    ...
    public static <T> List<Tree<T>> topMenu(List<Tree<T>> nodes, String idParam) {
        List<Tree<T>> topMenu = nodes.stream().filter(tree -> {
            return idParam.equals(tree.getParentId());
    ...
    public static <T> Tree<T> getAllChild(List<Tree<T>> nodes, String id) {
        Tree<T> parentTree = nodes.stream().filter(tree -> {
            return id.equals(tree.getId());
        }).collect(Collectors.toList()).get(0);
        List<Tree<T>> childs = nodes.stream().filter(tree -> {
            return tree.getParentId().equals(id);
        }).collect(Collectors.toList());
		...
		try {
			...
			for (T  tre : list) {
				...
				Arrays.stream(fields).forEach(field -> {
					...
					try {
						if(name.equals("deptId")||name.equals("id")||name.equals("orgId")){
							Object id = field.get(tre);
							tree.setId(id.toString());
							tree.setValue(Integer.parseInt(id.toString()));
						}else  if(name.equals("pid")||name.equals("parentId")||name.equals("pId")){
							Object pid = field.get(tre);
							tree.setParentId(pid.toString());
						}else  if(name.equals("name")||name.equals("orgName") || name.equals("subName")){
							Object names = field.get(tre);
							tree.setText(names.toString());
							tree.setLabel(names.toString());
						}else if(name.equals("createDate")){
							Object createDate = field.get(tre);
							tree.setCreateDate((Date)createDate);
						}
					}catch (Exception e){
