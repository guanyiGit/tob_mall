...
//        scan.close();
    ...
    public static List<String> orgReverseTwo(List<OrgInfo> list, int orgId, List<String> res) {
        ...
        OrgInfo orgInfo = list.stream().filter(org -> {
            return args[0].equals(org.getId() + "");
        ...
        List<OrgInfo> collect = list.stream().filter(inf -> {
            return args[0].equals(inf.getPid() + "");
        }).collect(Collectors.toList());
        collect.stream().forEach(org -> {
            Integer orgId1 = org.getId();
            orgReverseTwo(list, orgId1, res);
        });
        return res;
    }
    ...
    public static List<Integer> orgReverse(List<OrgInfo> list, int orgId, List<Integer> res) {
        ...
        List<OrgInfo> collect = list.stream().filter(inf -> {
            return args[0] == inf.getPid();
        }).collect(Collectors.toList());
        collect.stream().forEach(org -> {
            Integer orgId1 = org.getId();
            orgReverse(list, orgId1, res);
        });
        return res;
    }
