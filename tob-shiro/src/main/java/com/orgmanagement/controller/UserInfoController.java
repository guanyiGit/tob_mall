...
//		}
	...
	@PostMapping("/resetPassword/{oldPassWord}/{newPassword}")
	@ResponseBody
	R resetPassword(@PathVariable String oldPassWord, @PathVariable String newPassword){
		try {
			UserDO user = getUser();
			userService.resetPassword(user, oldPassWord, newPassword);
			return R.ok();
		} catch (Exception e){
