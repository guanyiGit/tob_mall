<template>
  ...
  <div class="wrap1 ww100 pdt30 pdl20 pdr20 pdb30">
          <p class="fs18 h28 lh28 cor0">订单审核</p>
          <ul class="ul003 mgt20">
            <li class="active" @click.prevent="checkTab = 0"><a style="cursor:pointer;">待审核</a></li>
            <li id="yi" @click.prevent="checkTab = 1"><a style="cursor:pointer;">已审核</a></li>
          </ul>
              ...
              <li class="db">
                ...
                <div v-for="(item,index) in list1" :key="index">
                  <div class="pdt20 pdb10 fs12 cor6 ovh pdl20">
                    ...
                    <span class="fr fs12 cor6 aside3"><!--<font v-bind:class="{ red: isRed }">本月预算超额</font>--> <a href="javascript:void(0)" class="dinb cor0289E8 mgl20 " @click="getDetail(item)">预算执行明细</a></span>
                  </div>
                  <table class="tab30" v-if="item.childOrders && item.childOrders.length>0">
                    ...
                    <tr>
                      ...
                      <td class="ww10">
                        <router-link style="cursor:pointer;" class="db w60 btn btn-xs btn-success mgt10" :to="{name:'pa_auditApproval',params:{orderInfo:JSON.stringify(item),type:0,excess:isExcess}}">审核确认</router-link>
                        <!--<a style="cursor:pointer;" class="db w60 btn btn-xs btn-success mgt10" @click="judgeExcess(item)">审核确认</a>-->
                      </td>
                    </tr>
                    </tbody>
                  </table>
  ...
  var _this = '';
    export default {
      ...
      data(){
          return{
            checkTab:0,
            parms:{userId:null, orgId:null, status:0, input:null, month:null, num:1, size:2},
            parms1:{userId:null, orgId:null, status:11, input:null, month:36, num:1, size:2},
      ...
      methods:{
        setOrderInfo(item,tabl){
          ...
          sessionStorage.setItem('namee','orderAudit');
          sessionStorage.setItem('tabl',tabl);
          this.$router.push({name:'pa_orderDetail'});
        },
        selectMyOrderss(){
          this.parms.num=1
          this.parms1.num=1
        ...
        //       for (var i=0;i<this.budgetList;i++) {
        //         if (this.budgetList[i].actualMonthOrSeasonRate-this.budgetList[i].planMonthOrSeasonRate>0) {
        //           this.isExcess=1
        //         }
        //       }
        //     }
        //     this.$router.push({name:'pa_auditApproval',params:{orderInfo:JSON.stringify(e),type:0,excess:this.isExcess}});
        //   }).catch((err)=>{
        //     console.log(err);
        //   });
        // },
